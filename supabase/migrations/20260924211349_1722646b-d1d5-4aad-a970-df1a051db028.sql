ALTER TABLE public.cask_transfers ADD COLUMN IF NOT EXISTS blockchain_tx_hash text;

CREATE OR REPLACE FUNCTION public.get_cask_provenance(_cask_id uuid)
 RETURNS TABLE(event_at timestamp with time zone, event_type text, title text, detail text, amount numeric, owner_label text, tx_hash text, on_chain boolean)
...
AS $function$
  ...
  sales AS (
    SELECT t.completed_at AS at, CASE WHEN t.sale_listing_id IS NOT NULL THEN 'resale' ELSE t.transaction_type END AS transaction_type, t.total_amount, t.buyer_id, t.blockchain_transaction_hash AS h,
  ...
  SELECT * FROM (
    ...
    SELECT s.at, CASE WHEN s.transaction_type ILIKE '%resale%' OR s.transaction_type ILIKE '%secondary%' THEN 'resale' ELSE 'sale' END,
           ...
           CASE WHEN s.h ~ '^0x[0-9a-fA-F]{64}$' THEN s.h END, coalesce(s.h ~ '^0x[0-9a-fA-F]{64}$', false)
    FROM sales s
    UNION ALL
    SELECT coalesce(ct.created_at, ct.transfer_date::timestamptz), 'transfer', initcap(replace(ct.transfer_type::text,'_',' ')),
           ...
           NULL, NULL,
           CASE WHEN ct.blockchain_tx_hash ~ '^0x[0-9a-fA-F]{64}$' THEN ct.blockchain_tx_hash ELSE ct.doc_hash END,
           coalesce(ct.blockchain_tx_hash ~ '^0x[0-9a-fA-F]{64}$', false)
